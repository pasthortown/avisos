...
CREATE PROCEDURE [Avisos].[pa_errorVacacionesGeneralDennisRepara]
AS
DECLARE
@fi DATE,
@ff DATE
...
SET DATEFORMAT dmy;
	...
	BEGIN
			...
			BEGIN TRY
	            ...
	            --  select * from  #tmp_errores2

				   declare @id int, @compania char(2), @trabajador char(10), @codigo varchar(20), @ciclo varchar(10)
				   

				 Declare  C4 Cursor local For                          
					 Select id, codigo  from #tmp_errores2 

				Open C4                      
                          
				 While @@Fetch_Status < 1                          
				  Begin                          
					 Fetch C4 Into  @id , @codigo                    
					 ...
					 If @@Fetch_Status <> 0                           
						...
						select @compania = right(@codigo,2)
						set @trabajador = left(@codigo,10)
						select @ciclo = ciclo_laboral from Vacacion.Solicitud_Vacaciones_Preprogramacion
						Where id = @id

						Insert into Vacacion.Solicitud_Vacaciones_Preprogramacion
						select id, compania, trabajador, ciclo_laboral, sec_prog_vac, tiempo_prog_vac, fecha_ini_per_vac, fecha_fin_per_vac, fecha_pago_vac, situacion_programa, tipo_vacaciones, manejo_dias_vac
                        from Vacacion.Solicitud_Vacaciones_PreprogramacionDiarios
						 where id = @id

						Insert into adam.dbo.programacion_vacaciones
						Select compania, trabajador, ciclo_laboral, sec_prog_vac, tiempo_prog_vac, fecha_ini_per_vac, 
						fecha_fin_per_vac, fecha_pago_vac, situacion_programa, tipo_vacaciones, manejo_dias_vac
                          from  Vacacion.Solicitud_Vacaciones_Preprogramacion
						  where id = @id


						  exec  vacacion.pa_validaReparaSaldosVacaciones @compania, @ciclo, @trabajador
